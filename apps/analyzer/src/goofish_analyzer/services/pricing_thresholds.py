...
def evaluate_pricing_availability(
    *,
    ...
    recency_weighted_sample_count: float | None = None,
    mad: float | None = None,
    confidence_score: float | None = None,
    confidence_reasons: list[str] | tuple[str, ...] | None = None,
    quality_tier: str | None = None,
    p15_price: float | None = None,
    p35_price: float | None = None,
    p50_price: float | None = None,
    latest_seen_at: str | datetime | None = None,
    ...
    evidence = {
        ...
        "effectiveSampleCount": float(effective_sample_count or 0.0),
        "recencyWeightedSampleCount": float(recency_weighted_sample_count or 0.0),
        "mad": float(mad) if mad is not None else None,
        "confidenceScore": float(confidence_score or 0.0),
        "confidenceReasons": [str(reason) for reason in (confidence_reasons or []) if str(reason).strip()],
        "qualityTier": str(quality_tier).strip() if quality_tier else None,
        "p15Price": float(p15_price) if p15_price is not None else None,
        "p35Price": float(p35_price) if p35_price is not None else None,
        "p50Price": float(p50_price) if p50_price is not None else None,
...
def build_pricing_availability_summary(
    *,
    ...
    return {
        ...
        "effectiveSampleCount": evidence_payload.get("effectiveSampleCount"),
        "recencyWeightedSampleCount": evidence_payload.get("recencyWeightedSampleCount"),
        "mad": evidence_payload.get("mad"),
        "confidenceScore": evidence_payload.get("confidenceScore"),
        "confidenceReasons": list(evidence_payload.get("confidenceReasons") or []),
        "qualityTier": evidence_payload.get("qualityTier"),
        "p15Price": evidence_payload.get("p15Price"),
        "p35Price": evidence_payload.get("p35Price"),
        "p50Price": evidence_payload.get("p50Price"),
...
def evaluate_pricing_row_availability(
    ...
    *,
    ...
    return evaluate_pricing_availability(
        ...
        p50_price=row.get("p50_price") or row.get("median_price"),
