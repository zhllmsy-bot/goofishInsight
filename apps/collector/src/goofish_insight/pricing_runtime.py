...
from .application.services.pricing_support import (
...
def _heuristic_condition_adjustment_from_tags(tags: list[str]) -> dict[str, Any] | None:
    if not tags:
        return None
    joined = " ".join(tags)
    ...
    if any(token in joined for token in ("划痕", "磕碰", "磨损", "使用痕迹", "掉漆")):
        return {
            "conditionCode": "fair",
            "conditionLabel": "Fair",
            "multiplier": Decimal("0.850"),
            "source": "heuristic",
        }
    if any(token in joined for token in ("故障", "不开机", "损坏", "进水", "碎屏", "维修")):
        return {
            "conditionCode": "parts_only",
            "conditionLabel": "Parts Only",
            "multiplier": Decimal("0.700"),
            "source": "heuristic",
        }
    return {
        "conditionCode": "used_unspecified",
        "conditionLabel": "Used",
        "multiplier": Decimal("0.900"),
        "source": "heuristic",
    }


def _resolve_condition_adjustment(
    item: Item,
    *,
    condition_adjusters: list[Any] | None = None,
) -> dict[str, Any] | None:
    tags = [str(tag).strip().lower() for tag in (item.condition_tags or []) if str(tag).strip()]
    if not tags:
        return None
    if condition_adjusters:
        configured = resolve_condition_adjuster_from_tags(tags=tags, adjusters=condition_adjusters)
        if configured is not None:
            return configured
    return _heuristic_condition_adjustment_from_tags(tags)
...
def build_pricing_sample_snapshot(
    *,
    ...
    condition_adjustment = _resolve_condition_adjustment(
        item,
        condition_adjusters=condition_adjusters,
    )
    condition_multiplier = (
        Decimal(str(condition_adjustment.get("multiplier")))
        if condition_adjustment is not None and condition_adjustment.get("multiplier") is not None
        else None
    )
    ...
    sample_payload = {
        ...
        "conditionCode": condition_adjustment.get("conditionCode") if condition_adjustment else None,
        "conditionLabel": condition_adjustment.get("conditionLabel") if condition_adjustment else None,
        "conditionSource": condition_adjustment.get("source") if condition_adjustment else None,
        "fingerprintHash": fingerprint_hash,
        "sampleQualityScore": sample_quality_score,
        "missingRequiredAttrs": list(missing_required_attrs),
    }
...
def _load_pricing_records_from_session(
    *,
    ...
    condition_adjusters_by_scope: dict[str, list[Any]] = {}
    for item, spec, spec_schema in rows:
        item_category_code = resolve_category_code(item.business_domain)
        if item_category_code and item_category_code not in condition_adjusters_by_scope:
            condition_adjusters_by_scope[item_category_code] = load_condition_adjusters_by_scope_with_session(
                session,
                scope_key=item_category_code,
            )
        condition_adjusters = condition_adjusters_by_scope.get(item_category_code or "")
    ...
    *,
    ...
    def _build_report(session_obj) -> dict[str, Any]:
        ...
        for item, spec, spec_schema in rows:
            ...
            if item_category_code not in condition_adjusters_by_scope:
                condition_adjusters_by_scope[item_category_code] = load_condition_adjusters_by_scope_with_session(
                    session_obj,
                    scope_key=item_category_code,
                )
            ...
            snapshot = build_pricing_sample_snapshot(
                ...
                spec=spec,
                spec_schema=spec_schema,
                condition_adjusters=condition_adjusters_by_scope.get(item_category_code),
            )
...
def resolve_pricing_record(
    ...
    *,
    ...
    sample_snapshot = build_pricing_sample_snapshot(
        item=item,
        spec=spec,
        spec_schema=spec_schema,
        record=record,
    ...
    record["sample_snapshot"] = dict(sample_snapshot.sample_payload)
    return record
...
def summarize_pricing_group(
    *,
    ...
    sample_state_counter: Counter[str] = Counter()
    for record in group_records:
        sample_snapshot = dict(record.get("sample_snapshot") or {})
        fingerprint_hash = str(sample_snapshot.get("fingerprintHash") or "").strip()
        if fingerprint_hash:
            fingerprint_counter[fingerprint_hash] += 1
        sample_state = str(sample_snapshot.get("sampleState") or "").strip()
        if sample_state:
            sample_state_counter[sample_state] += 1
    ...
    dominant_fingerprint_hash = fingerprint_counter.most_common(1)[0][0] if fingerprint_counter else None
    ...
    result = {
        ...
        "dominant_fingerprint_hash": dominant_fingerprint_hash,
        "sample_fingerprint_count": len(fingerprint_counter),
        "sample_state_counts": dict(sorted(sample_state_counter.items())),
    }
